...
CMAP_NDVI = clr.LinearSegmentedColormap.from_list(
    "ndvi",
    [
        "#cbbe9a",
        "#fffde4",
...
def text_phantom(text, width):  # TODO move to generic function
    ...
    # Get text dimensions using the new method (PIL 10.0.0+)
    try:
        # Try the new method first (PIL 10.0.0+)
        bbox = pil_font.getbbox(text)
        text_width = bbox[2] - bbox[0]
        text_height = bbox[3] - bbox[1]
    except AttributeError:
        # Fallback to the old method for older PIL versions
...
def veg_colorize(
    data, mask=None, clouds=None, setting="en21x"
):  # TODO move to generic function or take from earthnet.tk
    # mask has 1 channel
    ...
    if mask is not None:
        in_data = np.ma.array(
            in_data, mask=copy.deepcopy(mask.reshape(-1)).detach().cpu().numpy()
        )

    if clouds is None:
        return (
            torch.as_tensor(cmap(in_data)[:, :3], dtype=data.dtype, device=data.device)
            .reshape(t, h, w, 3)
            .permute(0, 3, 1, 2)
        )
    else:
        ...
        return torch.stack(
            [
                torch.where(
                    ...
                    out[:, 0, ...],
                    torch.zeros_like(
                        out[:, 0, ...], dtype=data.dtype, device=data.device
                    ),
                ),
                torch.where(
                    clouds,
                    out[:, 1, ...],
                    torch.zeros_like(
                        out[:, 1, ...], dtype=data.dtype, device=data.device
                    ),
                ),
                torch.where(
                    clouds,
                    ...
                    0.1
                    * torch.ones_like(
                        out[:, 2, ...], dtype=data.dtype, device=data.device
                    ),
                ),
            ],
            dim=1,
...
def log_viz(
    tensorboard_logger, viz_data, batch, batch_idx, current_epoch, setting="en21x"
):
    ...
    if "landcover" in batch:
        ...
        if setting == "en21x":
            ...
            lc = (
                1 - ((lc >= 2).bool() & (lc <= 6).bool()).float()
            )  # TODO legacy have the real min_lc and max_lc
    ...
    for i, (preds, scores) in enumerate(viz_data):
        for j in range(preds.shape[0]):
            if setting == "en21x":
                ndvi = veg_colorize(
                    preds[j, ...].squeeze(),
                    mask=lc[j, ...].repeat(preds.shape[1], 1, 1),
                    setting=setting,
                )
                text = (
                    f"Cube: {scores[j]['name']} Veg Score: {scores[j]['veg_score']:.4f}"
                )
                ...
                ndvi = veg_colorize(
                    (preds[j, :, 3, ...] - preds[j, :, 2, ...])
                    / (preds[j, :, 3, ...] + preds[j, :, 2, ...] + 1e-6),
                    mask=(
                        None
                        if "landcover" not in batch
                        else lc[j, ...].repeat(preds.shape[1], 1, 1)
                    ),
                    setting=setting,
                )
                grid = torchvision.utils.make_grid(ndvi, nrow=nrow)
            ...
            ndvi = (
                ...
                / (preds[j, :, 3, ...] + preds[j, :, 2, ...] + 1e-6)
                if setting == "en21-std"
                else preds[j, ...].squeeze()
            )
            ndvi_chg = (
                ndvi[1:, ...] - ndvi[:-1, ...] + 1
            ) / 2  # difference between 2 consecutive predictions.
            ...
            if i == 0:
                if setting == "en21x":
                    ...
                    ndvi = veg_colorize(
                        targs[j, :, 0, ...],
                        mask=lc[j, ...].repeat(targs.shape[1], 1, 1),
                        clouds=masks[j, :, 0, ...],
                        setting=setting,
                    )
                    ...
                    ndvi = veg_colorize(
                        ...
                        mask=(
                            None
                            if "landcover" not in batch
                            else lc[j, ...].repeat(targs.shape[1], 1, 1)
                        ),
