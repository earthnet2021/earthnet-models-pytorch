...
class EarthNet2022Dataset(Dataset):
    def __init__(self, folder: Union[Path, str], fp16=False):
        ...
        self.meteo_scaling_cube = xr.DataArray(
            data=[
                1e-4,
                1e-4,
                1e-4,
            ...
            coords={"variable": self.meteo_vars},
        )

        mean_meteo = torch.tensor(
            [
                -1.3039e00,
                -1.8422e00,
                -2.3154e00,
...
class EarthNet2022DataModule(pl.LightningDataModule):
    def __init__(self, hparams: argparse.Namespace):
        ...
        self.earthnet_test = EarthNet2022Dataset(
            self.base_dir / "test", fp16=self.hparams.fp16
        )
    ...
    def setup(self, stage: str = None):
        if stage == "fit" or stage is None:
            self.earthnet_train = EarthNet2022Dataset(
                self.base_dir / "train", fp16=self.hparams.fp16
            )

            self.earthnet_val = EarthNet2022Dataset(
                self.base_dir / "val", fp16=self.hparams.fp16
            )
