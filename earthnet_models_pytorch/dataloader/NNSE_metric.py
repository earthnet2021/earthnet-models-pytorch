...
class NormalizedNashSutcliffeEfficiency(Metric):
    ...
    def update(self, preds, targs, just_return = False):  
        ...
        if len(targs["dynamic_mask"]) > 0:
            ...
            masks = torch.where(masks.bool(), ((lc >= self.lc_min).bool() & (lc <= self.lc_max).bool()).type_as(masks).unsqueeze(1).repeat(1, preds.shape[1], 1, 1, 1), masks)
        # else define a mask using the landcover type
        else:
            masks = ((lc >= self.lc_min).bool() & (lc <= self.lc_max).bool()).type_as(preds).unsqueeze(1)
        ...
        if len(masks.shape) == 5:
            nse = 1 - torch.pow(preds * masks - targets * masks, 2).sum((1,2,3,4)) / torch.var(targets * masks, (1,2,3,4))
            nnse = 1 / (2 - nse)
            n_obs = (masks != 0).sum((1,2,3,4))
        else:
            # pixelwise models
            nse = 1 - torch.pow(preds * masks - targets * masks, 2).sum((1,2)) / torch.var(targets * masks, (1,2))
            nnse = 1 / (2 - nse)
        ...
        if just_return:
            ...
            return [{"name":  cubenames[i], "rmse": veg_score[i]} for i in range(len(cubenames))]
        else:
