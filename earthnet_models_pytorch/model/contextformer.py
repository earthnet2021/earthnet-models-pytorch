...
class ContextFormer(nn.Module):
    ...
    def __init__(self, hparams):
        ...
        if self.hparams.pvt:
            self.embed_images = PVT_embed(in_channels = self.hparams.n_image, out_channels = self.hparams.n_hidden, pretrained = True, frozen = self.hparams.pvt_frozen)
        else:
            self.embed_images = Mlp(in_features=self.hparams.n_image * self.hparams.patch_size * self.hparams.patch_size, hidden_features=self.hparams.n_hidden, out_features=self.hparams.n_hidden)

        self.embed_weather = Mlp(in_features=self.hparams.n_weather, hidden_features=self.hparams.n_hidden, out_features=self.hparams.n_hidden)
        ...
        self.head = Mlp(in_features=self.hparams.n_hidden, hidden_features=self.hparams.n_hidden, out_features=self.hparams.n_out * self.hparams.patch_size * self.hparams.patch_size)

        if self.hparams.predict_delta_avg or self.hparams.predict_delta_max:
            self.head_avg = Mlp(in_features=self.hparams.n_hidden, hidden_features=self.hparams.n_hidden, out_features=self.hparams.n_out * self.hparams.patch_size * self.hparams.patch_size)
    ...
    def forward(self, data, pred_start: int = 0, preds_length: Optional[int] = None):
        ...
        if self.hparams.pvt:
            ...
            image_patches = images.reshape(B, T, C, H//self.hparams.patch_size,self.hparams.patch_size,W//self.hparams.patch_size,self.hparams.patch_size).permute(0,3,5,1,2,4,6).reshape(B * H//self.hparams.patch_size * W//self.hparams.patch_size, T, C * self.hparams.patch_size * self.hparams.patch_size)
            B_patch, N_patch, C_patch = image_patches.shape
            image_patches_embed = self.embed_images(image_patches)

        

        mask_patches = hr_dynamic_mask.reshape(B, T, 1, H//self.hparams.patch_size,self.hparams.patch_size,W//self.hparams.patch_size,self.hparams.patch_size).permute(0,3,5,1,2,4,6).reshape(B * H//self.hparams.patch_size * W//self.hparams.patch_size, T, 1 * self.hparams.patch_size * self.hparams.patch_size)

        weather_patches = weather.reshape(B, 1, t_m, c_m).repeat(1, H//self.hparams.patch_size * W//self.hparams.patch_size, 1, 1).reshape(B_patch, t_m, c_m)
        ...
        if self.hparams.mtm and self.training and ((torch.rand(1) <= self.hparams.p_use_mtm ).all()):
            token_mask = (torch.rand(B_patch, N_patch,device=weather_patches.device) < self.hparams.p_mtm).type_as(weather_patches).reshape(B_patch, N_patch, 1).repeat(1, 1, self.hparams.n_hidden)
            token_mask[:,:self.hparams.leave_n_first] = 0

        else:
            token_mask = torch.ones(B_patch, N_patch,device=weather_patches.device).type_as(weather_patches).reshape(B_patch, N_patch, 1).repeat(1, 1, self.hparams.n_hidden)
            token_mask[:,:c_l] = 0


        image_patches_embed[token_mask.bool()] = (self.mask_token).reshape(1, 1, self.hparams.n_hidden).repeat(B_patch, N_patch, 1)[token_mask.bool()]

        if self.hparams.mask_clouds:
            cloud_mask = (mask_patches.max(-1, keepdim = True)[0] > 0).bool().repeat(1,1,self.hparams.n_hidden)
            image_patches_embed[cloud_mask] = (self.mask_token).reshape(1, 1, self.hparams.n_hidden).repeat(B_patch, N_patch, 1)[cloud_mask]
        ...
        pos_embed = get_sinusoid_encoding_table(N_patch, self.hparams.n_hidden).to(patches_embed.device).unsqueeze(0).repeat(B_patch, 1, 1)
        ...
        x_out[~token_mask.bool()[:,:,:self.hparams.n_out*self.hparams.patch_size*self.hparams.patch_size]] = -1

        # unpatchify images
        images_out = x_out.reshape(B, H//self.hparams.patch_size, W//self.hparams.patch_size, N_patch, self.hparams.n_out, self.hparams.patch_size, self.hparams.patch_size).permute(0, 3, 4, 1, 5, 2, 6).reshape(B, N_patch, self.hparams.n_out, H, W)

        if self.hparams.add_last_ndvi:
            mask = hr_dynamic_mask[:, :c_l,...]
            
            indxs = torch.arange(c_l, device = mask.device).expand(B, self.hparams.n_out, H, W, -1).permute(0, 4, 1, 2, 3)

            ndvi = hr_dynamic_inputs[:, :c_l, :self.hparams.n_out,...]

            last_pixel = torch.gather(ndvi, 1, (indxs * (mask < 1)).argmax(1, keepdim = True)) 
            ...
            mean_ndvi = ((ndvi * (mask < 1)).sum(1, keepdim = True) / ((mask < 1).sum(1, keepdim = True) + 1e-8)).clamp(min = -1.0, max = 1.0)
        ...
        if self.hparams.predict_delta_avg:
            ...
            image_avg_out =  image_avg.reshape(B, H//self.hparams.patch_size, W//self.hparams.patch_size, 1, self.hparams.n_out, self.hparams.patch_size, self.hparams.patch_size).permute(0, 3, 4, 1, 5, 2, 6).reshape(B, 1, self.hparams.n_out, H, W)
        ...
        if self.hparams.spatial_shuffle:
            ...
            images_out = images_out.permute(1, 2, 0, 3, 4).reshape(T, C, B*H*W)[:, :, invperm].reshape(T, C, B,H,W).permute(2, 0, 1, 3, 4)
