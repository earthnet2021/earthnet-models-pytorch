...
class SpatioTemporalUNet(nn.Module):
    ...
    def __init__(self, hparams):
        ...
        self.n_tokens_c = 8
        c_channels = self.hparams.n_weather_inputs * (self.hparams.context_length+ self.hparams.target_length)
        ...
        self.enc, self.dec = get_encoder_decoder("PatchMerge", enc_in_channels, self.hparams.hid_channels, self.hparams.target_length, down_factor=2**self.hparams.enc_depth, filter_size = self.hparams.filter_size, skip_connection=True, norm = self.hparams.norm, act = self.hparams.act, bias = self.hparams.bias, readout_act=self.hparams.readout_act,conditioning = enc_dec_proc_conditioning, c_channels_enc = c_channels,c_channels_dec = c_channels, n_tokens_c = self.n_tokens_c)
