...
class EarthNet2021XOldDataset(Dataset):
    def __init__(
        ...
        self.eobs_scaling_cube = xr.DataArray(
            data=[
                100,
                100,
                100,
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        kndvi = minicube.kndvi.values.transpose((2, 0, 1))[:, None, ...].astype(
            self.type
        )  # t c h w

        kndvi /= np.tanh(
            1
        )  # WARNING TODO compute the distance between all pixels for sigma (respect the Gustau advice)
        ...
        if not self.eobs_spread:
            eobs_cube = eobs_cube.sel(
                variable=[v for v in self.eobs_vars if "spread" not in v]
            )
        ...
        if self.soilgrids_all:
            sg_cube = minicube[
                sorted([v for v in minicube.variables if v.startswith("sg")])
            ].to_array()
        else:
            sg_cube = minicube[
                sorted([v for v in minicube.variables if v.startswith("sg")])
            ...
            for var in [
                ...
                sg_cube[f"sg_{var}_0-30cm_median"] = (
                    sg_cube[f"sg_{var}_0-5cm_Q0.5"] / 6
                    + sg_cube[f"sg_{var}_5-15cm_Q0.5"] / 3
                    + sg_cube[f"sg_{var}_15-30cm_Q0.5"] / 2
                )
                sg_cube[f"sg_{var}_30-200cm_median"] = (
                    (3 / 17) * sg_cube[f"sg_{var}_30-60cm_Q0.5"]
                    + (4 / 17) * sg_cube[f"sg_{var}_60-100cm_Q0.5"]
                    + (10 / 17) * sg_cube[f"sg_{var}_100-200cm_Q0.5"]
                )
            sg_cube = sg_cube[
                sorted([v for v in sg_cube.variables if v.endswith("median")])
            ].to_array()
...
class EarthNet2021XpxOldDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        if self.soilgrids_all:
            sg_cube = pixel[
                sorted([v for v in pixel.variables if v.startswith("sg")])
            ].to_array()
        else:
...
class EarthNet2021XpxOldDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: str = None):
        if stage == "fit" or stage is None:
            ...
            try:  # PyTorch 1.5 safe....
                _, self.earthnet_val = random_split(
                    earthnet_corpus,
                    [train_size, val_size],
                    generator=torch.Generator().manual_seed(
                        int(self.hparams.val_split_seed)
