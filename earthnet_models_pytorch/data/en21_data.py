...
class EarthNet2021Dataset(Dataset):
    ...
    def __init__(
        ...
        fp16=False,
        landcover_folder: Optional[Union[Path, str]] = None,
        remove_first_n_steps=0,
    ):
        if not isinstance(folder, Path):
            folder = Path(folder)
        assert not {"target", "context"}.issubset(
            set([d.name for d in folder.glob("*") if d.is_dir()])
        )
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        highresdynamic = np.transpose(npz["highresdynamic"], (3, 2, 0, 1)).astype(
            self.type
        )[self.t_0 :, ...]
        ...
        if self.noisy_masked_pixels:
            ...
            all_pixels = np.stack(
                int(images.size / all_pixels.size + 1) * [all_pixels], axis=1
            )
            ...
            images = np.where(
                np.transpose(masks, (1, 0, 2, 3)) == 0, all_pixels, images
            )
            images = np.transpose(images, (1, 0, 2, 3))

        if self.use_meso_static_as_dynamic:
            mesodynamic = np.concatenate(
                [
                    mesodynamic,
                    mesostatic[np.newaxis, :, :, :].repeat(mesodynamic.shape[0], 0),
                ],
                axis=1,
            )

        data = {
            "dynamic": [torch.from_numpy(images), torch.from_numpy(mesodynamic)],
            "dynamic_mask": [torch.from_numpy(masks)],
            "static": (
                [torch.from_numpy(highresstatic), torch.from_numpy(mesostatic)]
                if not self.use_meso_static_as_dynamic
                else [torch.from_numpy(highresstatic)]
            ),
...
class EarthNet2021DataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: str = None):
        ...
        if stage == "fit" or stage is None:
            earthnet_corpus = EarthNet2021Dataset(
                self.base_dir / "train",
                ...
                landcover_folder=(
                    Path(self.hparams.landcover_folder) / "train"
                    if self.hparams.landcover_folder is not None
                    else None
                ),
            )
        ...
        if stage == "test" or stage is None:
            self.earthnet_test = EarthNet2021Dataset(
                self.base_dir / self.__TRACKS__[self.hparams.test_track][0],
                noisy_masked_pixels=self.hparams.noisy_masked_pixels,
                use_meso_static_as_dynamic=self.hparams.use_meso_static_as_dynamic,
                fp16=self.hparams.fp16,
                landcover_folder=(
                    Path(self.hparams.landcover_folder)
                    / self.__TRACKS__[self.hparams.test_track][1]
                    if self.hparams.landcover_folder is not None
                    else None
                ),
                remove_first_n_steps=(
                    0
                    if self.hparams.test_track != "full_sea"
                    else self.hparams.full_sea_remove_first_n_steps
                ),
            )
