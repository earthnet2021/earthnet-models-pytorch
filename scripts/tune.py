...
class Tuner:
    ...
    def create_tune_cfgs(self, params_file, setting_file):
        ...
        flat_params = [
            [{"param": param_name, "val": v} for v in param_vals]
            for param_name, param_vals in params_dict.items()
        ]
    ...
    def start_one_trial(self, trial_path):
        ...
        if not self.slurm:
            ...
            cmd = [
                "sbatch",
                f"{script_path/'slurmrun.sh'}",
                trial_path,
                "train",
            ]  # f"{script_path/'slurmrun.sh'} {trial_path} train"
    ...
    def run_trials(self):
        ...
        if self.slurm:
            ...
            for idx, manager in enumerate(managers):
                ...
                job = str(message).split(" ")[-1][
                    :-3
                ]  # Extracting Job ID that is outputed when running sbatch
                jobs.append(job)
        ...
        while not all(done):
            ...
            for idx, manager in enumerate(managers):
                if not self.slurm:
                    ...
                    state = (
                        str(check.stdout).split("\\n")[-2].strip()
                    )  # Extracting State from SLURM sacct command...
                    if state == "COMPLETED":
                        done[idx] = True
    ...
    def aggregate_results(self):
        ...
        best_score = (
            max(best_scores)
            if setting_dict["Setting"] == "en21-std"
            else min(best_scores)
        )
        best_trial = [
            self.trial_paths[i] for i, s in enumerate(best_scores) if (s == best_score)
        ][0]
        ...
        scores_df.to_csv(
            Path(best_setting["Logger"]["save_dir"])
            / best_setting["Logger"]["name"]
            / "tune_results.csv"
        )
